        ...
        <div className="container mx-auto px-4 md:px-6 h-20 flex items-center justify-between">
          ...
          <Link href="/" className="flex items-center gap-3 md:mr-0 absolute left-1/2 -translate-x-1/2 md:static md:translate-x-0">
            <div className="w-10 h-10 md:w-14 md:h-14 flex items-center justify-center">
              <img src="/logo/agora-white.png" alt="الشعار" className="w-full h-full object-contain drop-shadow-md invert opacity-90" />
            </div>
            <div className="flex-col leading-none hidden sm:flex">
              <span className="text-xl font-black tracking-[0.2em] text-black">AROA</span>
              <span className="text-[9px] tracking-[0.45em] font-medium text-gray-500">PERFUMES</span>
            </div>
          </Link>

          {/* Desktop Nav */}
          <nav className="hidden md:flex items-center gap-8">
            <Link href="/" className="text-sm font-bold text-gray-800 hover:text-black transition-colors">الرئيسية</Link>
            
            <div className="relative group">
              <Link href="/#brands" className="text-sm font-bold text-gray-800 hover:text-black transition-colors flex items-center gap-1 py-4">
              ...
              <div className="absolute top-full right-0 w-64 bg-white border border-gray-100 shadow-xl rounded-2xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 transform origin-top-right scale-95 group-hover:scale-100 p-2 z-50">
                {brands.map(brand => (
                  <Link key={brand.id} href={`/search?brand=${brand.id}`} className="block px-4 py-3 text-sm text-gray-700 hover:bg-gray-50 hover:text-black rounded-xl transition-colors">
                    {brand.name}
                  </Link>
                ))}
              </div>
            ...
            <Link href="/#discounts" className="text-sm font-bold text-gray-800 hover:text-black transition-colors flex items-center gap-1">
              التخفيضات <Percent className="w-4 h-4 text-gray-600" />
            </Link>
            <Link href="/#packages" className="text-sm font-bold text-gray-800 hover:text-black transition-colors flex items-center gap-1">
              البكجات <Package className="w-4 h-4 text-gray-600" />
            </Link>
          </nav>
            ...
            <div className="hidden md:block relative">
              <form onSubmit={handleSearch}>
                <input 
                  ...
                  className="w-48 lg:w-64 pl-10 pr-4 py-2 bg-gray-50 border border-gray-200 rounded-full text-sm focus:outline-none focus:ring-2 focus:ring-black focus:bg-white transition-all"
                />
                <button type="submit" className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-black">
        ...
        <div className="fixed inset-0 z-[100] md:hidden">
          ...
          <div className="absolute top-0 right-0 bottom-0 w-[85%] max-w-sm bg-white shadow-2xl animate-in slide-in-from-right flex flex-col">
            <div className="p-6 flex items-center justify-between border-b border-gray-100">
              <span className="text-xl font-black text-black tracking-widest">AROA</span>
                  ...
                  <div className="pl-4 pr-6 py-2 space-y-1 bg-gray-50 rounded-xl mb-2">
                    {brands.map(brand => (
                      <Link 
                        ...
                        className="block px-4 py-2 text-sm font-medium text-gray-600 hover:text-black transition-colors"
                      >
                        {brand.name}
                      </Link>
                    ))}
